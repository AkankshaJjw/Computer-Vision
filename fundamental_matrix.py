...
def point_line_distance(line, point):
    ...
    u, v, w = point
    num = a*u + b*v + c
    den = np.sqrt(np.square(a) + np.square(b))
    error = num/den
...
def signed_point_line_errors(x_0s, F, x_1s):
    ...
    errors = np.zeros((2*N))
    for i in range(N):
        Fx_1 = np.dot(F,x_1s[i])
        Ft = F.transpose()
        Ftx_0 = np.dot(Ft,x_0s[i])
        errors[2*i] = point_line_distance(Fx_1,x_0s[i])
        errors[2*i+1] = point_line_distance(Ftx_0,x_1s[i])
    errors = list(errors)
